...
import '../../Administrador/Pedidos/Pedidos.css';

import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { FaEye, FaSortUp, FaSortDown, FaFilePdf, FaCheck } from 'react-icons/fa';
...
const formatCurrency = (value: number) =>
  new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
...
const PedidosFacturador: React.FC = () => {
  ...
  const [modalFacturar, setModalFacturar] = useState<Pedido | null>(null); // Nuevo estado para facturar
  ...
  // Efecto para abrir pedido desde dashboard
  useEffect(() => {
    ...
    <div className="pedidos-page">
      <div className="pedidos-header">
        <h2 className="pedidos-title">
          <i className="bi bi-bag"></i> Gestión de Facturación
        </h2>
        {/* BOTÓN NUEVO PEDIDO ELIMINADO PARA EL FACTURADOR */}
      </div>
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <div className="pedidos-acciones">
                          {/* SIEMPRE DISPONIBLE: Ver Detalle */}
                          <button
                          ...
                          {/* CONFIRMADOS Y FACTURADOS: Imprimir PDF */}
                          {(p.estado === 'confirmado' || p.estado === 'facturado') && (
                            <button
                          ...
                          {/* SÓLO CONFIRMADOS: Cambiar a Facturado */}
                          {p.estado === 'confirmado' && (
                            <button
                              className="btn-accion"
                              title="Marcar como Facturado"
                              onClick={() => setModalFacturar(p)}
                              style={{ background: '#10b981', color: '#fff' }} // Verde para la acción de facturar
                            >
        ...
        <div className="modal-overlay" onClick={() => setModalDetalle(null)}>
          <div className="modal-box modal-grande" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              {/* Opción rápida dentro del modal para facturar si está confirmado */}
              {modalDetalle.estado === 'confirmado' && (
                <button 
                  className="perfil-btn-guardar" 
                  style={{ background: '#10b981' }}
                  onClick={() => {
                    setModalFacturar(modalDetalle);
                    setModalDetalle(null); // Cierra el de detalle y abre el de confirmación
                  }}
                >
                  <FaCheck style={{ marginRight: '5px' }}/> Marcar como Facturado
                </button>
              )}
            </div>
          </div>
      ...
      {modalFacturar && (
        <div className="modal-overlay" onClick={() => setModalFacturar(null)}>
          <div className="modal-box modal-pequeño" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Facturar Pedido</h3>
              <button className="modal-cerrar" onClick={() => setModalFacturar(null)}>✕</button>
            </div>
            <div className="modal-body">
              <p>¿Deseas registrar como <strong>Facturado</strong> el pedido <span className="ref-tag">{modalFacturar.referencia}</span> de <strong>{modalFacturar.cliente_nombre}</strong>?</p>
              <p className="pedidos-aviso" style={{ color: '#10b981' }}>Al confirmar, el pedido quedará cerrado contablemente.</p>
            </div>
            <div className="modal-footer">
              <button className="btn-modal-cancelar" onClick={() => setModalFacturar(null)}>Cancelar</button>
              <button className="perfil-btn-guardar" onClick={handleFacturar} disabled={guardando} style={{ background: '#10b981' }}>
                {guardando ? 'Procesando...' : 'Sí, Facturar'}
              </button>
            </div>
          </div>
        </div>
...
export default PedidosFacturador;